"""
    Implementation of the stratified sampling
"""
from itertools import chain, repeat, product
import tensorflow as tf
import numpy as np
from vegasflow.configflow import DTYPE, fone, fzero, DTYPEINT
...
class StratifiedFlow(MonteCarloFlow):
    ...
    def __init__(self, n_dim, n_events, adaptive=True, **kwargs):
        ...
        self.n_strat = tf.math.floor(tf.math.pow(n_events/N_STRAT_MIN,
                                                 1/n_dim))
        substratification_np = np.linspace(0, 1, int(self.n_strat)+1)
        stratifications_np = substratification_np.repeat(n_dim).reshape(-1,
                                                                        n_dim).T
    ...
    def _run_event(self, integrand, ncalls=None):
        ...
        x, w, segm = generate_random_array(rnds, self.n_strat, self.n_ev,
                                           self.hypercubes)
        ...
        arr_var = None
        if self.adaptive:
            hypercube_volume = tf.cast(tf.math.pow(1/self.n_strat, self.n_dim),
                                       DTYPE)
            arr_var = ((ress2 *
                       tf.cast(self.n_ev, DTYPE) *
                       tf.square(hypercube_volume)) -
                       tf.square(ress*hypercube_volume))
    ...
    def _run_iteration(self):
        ...
        res = tf.reduce_sum(ress) / tf.cast(tf.math.pow(self.n_strat,
                                                        self.n_dim),
                                            DTYPE)

        sigma2 = tf.reduce_sum(sigmas2)/tf.cast(tf.math.pow(self.n_strat,
                                                            self.n_dim),
                                                DTYPE) / self.n_events
